...
class FPNObjective(BaseObjective):
    ...
    def batch_transform(self, inputs, outputs, annotations):
        ...
        anchors, ctr_anchors = self.anchors.forward(img_shape, device)
    ...
    def get_acc(self, inputs, outputs, targets, cand_net):
        ...
        return [torch.tensor(a) for a in accuracy(confidences, conf_t, topk=(1, 5))]
...
class Anchors(nn.Module):
    ...
    def forward(self, image_shape, device, dtype=torch.float32):
        ...
        self.anchors[image_shape] = anchor_boxes, ctr_anchors
        return anchor_boxes, ctr_anchors
...
class FocalLoss(nn.Module):
    ...
    def forward(self, predictions, annotations, anchors):
        ...
        for j in range(batch_size):
            ...
            targets = torch.ones_like(classification).to(device) * -1
            ...
            alpha_factor = torch.ones_like(targets).to(device) * alpha
            ...
            zeros = torch.zeros_like(cls_loss).to(device)
